...
impl StringUtil {
    ...
    pub fn result_str2result_string(
        source: Result<&str, Box<dyn Error>>,
...
impl BuffUtil {
    ...
    pub fn buff_to_str(buff: Vec<u8>) -> MQResult<String> {
        let s = String::from_utf8(buff).map_err(|e| {
            MQError::ConvertError(format!("the buff convert str failed.\n\terror: {}", e))
        })?;
        Ok(s)
    }
